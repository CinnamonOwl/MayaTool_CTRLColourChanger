# ------------------------------------------------------------------
# CTRL ColourChanger v003
# made with the help of Gemini
# ------------------------------------------------------------------
# Change Log:
# Added the side assignment colour thingy
# Made it save Json data into a file to load.
...
class CollapsibleGroup(QtWidgets.QWidget):
    ...
    def __init__(self, title, content_widget=None, parent=None, start_collapsed=False):
        ...
        # Use the passed widget (for Sides) or create a new List (for Presets)
...
class CTRLColourChanger(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # Inside class CTRLColourChanger -> def __init__
    ...
    def setup_side_assignment_ui(self):
        """Creates the Side Assignment section safely."""
        self.side_content = QtWidgets.QWidget()
        self.side_content.setStyleSheet("background-color: #2b2b2b; border-radius: 4px;")
        self.side_content.setFixedHeight(85) # Height of the internal area
        ...
        for side in ["Left", "Center", "Right"]:
            ...
            col.setSpacing(2)
            lbl = QtWidgets.QLabel(side); lbl.setAlignment(QtCore.Qt.AlignCenter)
            lbl.setStyleSheet("font-size: 10px; color: #aaa;")
            
            box = QtWidgets.QFrame()
            box.setFixedSize(60, 25) # EDIT SIZE HERE (Width, Height)
            box.setStyleSheet("background-color: #111; border: 1px solid #444; border-radius: 4px;")
            
            self.side_previews[side] = box
            col.addWidget(lbl); col.addWidget(box)
            boxes_lay.addLayout(col)
        
        lay.addLayout(boxes_lay)
        btn = QtWidgets.QPushButton("Apply Side Colours")
        btn.setFixedHeight(24); btn.setStyleSheet("font-weight: bold; background-color: #445566;")
        btn.clicked.connect(self.apply_side_logic)
        lay.addWidget(btn)

        # Create group and pass the side_content widget directly
        self.side_group = CollapsibleGroup("Side Assignments", content_widget=self.side_content, start_collapsed=True)
        self.main_layout.insertWidget(0, self.side_group)

    def set_side_color(self, side, color_data):
        self.side_colors[side] = color_data
        c = ColorUtils.get_ui_color_from_data(color_data)
        self.side_previews[side].setStyleSheet("background-color: %s; border: 1px solid #5285a6; border-radius: 4px;" % c.name())
    ...
    def apply_side_logic(self):
        ...
        for node in targets:
            ...
            col = self.side_colors["Left"] if x > 0.1 else self.side_colors["Right"] if x < -0.1 else self.side_colors["Center"]
            if col: MayaLogic.apply_smart_override([node], col)
        cmds.inViewMessage(amg="Side Colors Applied", pos="topCenter", fade=True)
    
    def get_settings_path(self):
        """Returns the path to the json file in your Maya user folder."""
        user_app_dir = cmds.internalVar(userAppDir=True)
        return os.path.join(user_app_dir, "scripts", "ctrl_colour_presets.json")

    def save_settings(self):
        """Saves current grouped_presets to the hard drive."""
        # First, ensure the data is synced from the UI
    ...
    def load_settings(self):
        """Loads presets from the hard drive if the file exists."""
        path = self.get_settings_path()
    ...
    def delete_and_save(self, list_widget, item):
        """Removes an item and updates the save file."""
        list_widget.takeItem(list_widget.row(item))
        self.save_settings()

    def open_json_folder_menu(self, pos):
        """Creates the right-click menu for the JSON button."""
        menu = QtWidgets.QMenu()
        menu.addAction("Open File Location").triggered.connect(self.reveal_json_in_explorer)
        
        # Maps the click position to the global screen position
        menu.exec_(self.btn_json.mapToGlobal(pos))

    def reveal_json_in_explorer(self):
        """Opens the OS file explorer at the JSON settings location."""
        path = self.get_settings_path()
        ...
        # Cross-platform way to open a folder in Windows/Mac/Linux
    ...
    def open_context_menu(self, list_widget, pos):
        ...
        if item:
            ...
            # --- NEW: SIDE ASSIGNMENT SUBMENU ---
            side_menu = menu.addMenu("Assign to Side...")
            side_menu.addAction("Set to Left").triggered.connect(partial(self.set_side_color, "Left", raw_data))
            side_menu.addAction("Set to Center").triggered.connect(partial(self.set_side_color, "Center", raw_data))
            side_menu.addAction("Set to Right").triggered.connect(partial(self.set_side_color, "Right", raw_data))
            menu.addSeparator()
            ...
            menu.addAction("Delete Item").triggered.connect(lambda: self.delete_and_save(list_widget, item))
        else:
